...
from .model import (
    CrossBioSAE, CrossBioSAEConfig, CrossBioSAELoss,
    StandardSAE, StandardSAEConfig, StandardSAELoss,
    count_parameters,
)
...
# =============================================================================
# StandardSAETrainer — single-modality, no cross-modal loss
# =============================================================================


class StandardSAETrainer:
    """
    Training loop for StandardSAE (single-modality).

    Simplified version of CrossBioSAETrainer:
    - Works with a single modality (protein OR dna)
    - Uses StandardSAE + StandardSAELoss
    - Same optimizer / LR schedule / gradient clipping / checkpoint / wandb interface
    - No cross-modal warmup
    """
    ...
    def __init__(
        ...
        self.config = config
        self.modality = modality  # "protein" or "dna"
        self.train_loader = train_loader
        self.val_loader = val_loader
        self.device = device
        self.max_steps = max_steps
        ...
        # Model
        self.model = StandardSAE(config).to(device)
        self.loss_fn = StandardSAELoss(config)

        # Log parameter counts
        param_counts = count_parameters(self.model)
        logger.info(f"StandardSAE [{modality}] parameter counts:")
        for name, count in param_counts.items():
            logger.info(f"  {name}: {count:,}")
        ...
        if self.use_wandb:
            wandb.init(
                project=wandb_project,
                name=wandb_run_name or f"standard_sae_{modality}",
                config={
                    "model": vars(config),
                    "modality": modality,
                    "training": {
    ...
    def train_step(self, batch: dict) -> dict[str, float]:
        ...
        # Extract activations for our modality
        acts_key = f"{self.modality}_acts"
        acts = batch[acts_key].to(self.device)

        # Update running mean
        if self.config.normalize_inputs:
            self.model.update_running_mean(acts)

        # Forward pass
        reconstructed, features = self.model(acts)

        # Compute losses
        losses = self.loss_fn(original=acts, reconstructed=reconstructed, features=features)
        ...
        # Sparsity metrics
        metrics["l0"] = (features > 0).float().sum(dim=-1).mean().item()
        metrics["feature_density"] = (features > 0).float().mean().item()
    ...
    def train(self):
        """Main training loop."""
        logger.info(
            f"Starting StandardSAE [{self.modality}] training: "
            f"{self._estimate_total_steps()} estimated steps, "
            f"{len(self.train_loader)} batches/epoch"
        )
        ...
        for epoch in range(self.epoch, self.max_epochs):
            ...
            for batch in self.train_loader:
                ...
                if self.global_step % self.log_interval == 0:
                    ...
                    logger.info(f"[{self.modality}] Step {self.global_step} | {metrics_str}")
                    if self.use_wandb:
                        wandb.log(
                            {f"train/{k}": v for k, v in metrics.items()},
                            step=self.global_step,
                ...
                if self.global_step % self.eval_interval == 0:
                    ...
                    if val_metrics:
                        ...
                        logger.info(
                            f"[{self.modality}] Validation @ step {self.global_step} | {val_str}"
                        )
                        if self.use_wandb:
                            wandb.log(val_metrics, step=self.global_step)
            ...
            epoch_time = time.time() - epoch_start
            logger.info(f"[{self.modality}] Epoch {epoch} completed in {epoch_time:.1f}s")

        self.save_checkpoint("final")
        logger.info(f"StandardSAE [{self.modality}] training complete")
