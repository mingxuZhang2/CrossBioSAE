#!/bin/bash
#SBATCH -J rad51c_native_sae
#SBATCH -p i64m1tga800u
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH -t 06:00:00
#SBATCH -o logs/rad51c_native_sae_%j.out
#SBATCH -e logs/rad51c_native_sae_%j.out

set -euo pipefail
echo "=== RAD51C native SAE $(hostname) $(date) ==="
...
python -u scripts/run_brca1_native_finetuned_sae_intervention.py \
  ...
  --run-name RAD51C \
  --output-prefix rad51c_native_finetuned_sae_intervention \
  --domain-col domains \
...
echo "RAD51C_NATIVE_SAE_DONE $(date)"
